...
class NotificationSettingsScreen extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Scaffold(
      ...
      body: SafeArea(
        child: _buildBody(context, notifService, bluetoothService, commonApps),
      ),
    );
  }

  Widget _buildBody(BuildContext context, NotificationService notifService,
      BluetoothService bluetoothService, List<Map<String, dynamic>> commonApps) {
    final content = Column(
      children: [
        _buildAllowAllToggle(context, notifService, commonApps),
        if (!notifService.allowAll)
          Padding(
            ...
            child: Text(
              ...
              style: TextStyle(fontSize: context.fontSize(9), color: Colors.white38),
            ),
          ),
        SizedBox(height: context.padding(4)),
        Expanded(child: _buildAppList(context, notifService, commonApps)),
        _buildSyncButton(context, notifService, bluetoothService),
      ],
    );

    return context.isRound ? RoundClip(child: content) : content;
  }

  Widget _buildAllowAllToggle(BuildContext context, NotificationService notifService,
      List<Map<String, dynamic>> commonApps) {
    return Padding(
          ...
          children: [
            ...
            Expanded(
              child: Column(
                ...
                children: [
                  Text('Allow All Apps',
                      style: TextStyle(fontSize: context.fontSize(11), fontWeight: FontWeight.bold)),
                  Text(
                    notifService.allowAll
                        ? 'All notifications will be forwarded'
                        : 'Only selected apps will forward',
                    style: TextStyle(fontSize: context.fontSize(8), color: Colors.white54),
                  ),
                ],
              ),
            ),
  ...
  Widget _buildAppList(
      BuildContext context, NotificationService notifService, List<Map<String, dynamic>> commonApps) {
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        return Container(
          margin: EdgeInsets.symmetric(vertical: context.padding(2), horizontal: context.padding(4)),
          decoration: BoxDecoration(
            color: isAllowed ? Colors.green.withValues(alpha: 0.08) : Colors.white.withValues(alpha: 0.03),
            borderRadius: BorderRadius.circular(12),
            border: Border.all(
              color: isAllowed ? Colors.green.withValues(alpha: 0.3) : Colors.white.withValues(alpha: 0.1),
            ),
          ),
          child: Material(
                ...
                child: Row(
                  children: [
                    Container(
                      ...
                      decoration: BoxDecoration(
                        color: isAllowed ? Colors.green.withValues(alpha: 0.2) : Colors.white.withValues(alpha: 0.1),
                        borderRadius: BorderRadius.circular(8),
                      ),
                      child: Icon(icon, size: context.iconSize(14),
                    ...
                    Icon(isAllowed ? Icons.check_circle : Icons.circle_outlined,
  ...
  Widget _buildSyncButton(BuildContext context, NotificationService notifService,
